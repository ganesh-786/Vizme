...
class VizmeClient {
  ...
  scheduleRetryWithBackoff() {
    ...
    const delay = Math.min(this.retryBaseMs * Math.pow(2, this.retryAttempt), 30000);
  ...
  constructor(config) {
    ...
    if (typeof window !== 'undefined' && window.addEventListener) {
      this._vizmeTrackHandler = (e) => {
        ...
        if (!event) return;
        const numValue =
          typeof value === 'number' && isFinite(value) ? value : parseFloat(value) || 0;
        if (operation === 'set') {
