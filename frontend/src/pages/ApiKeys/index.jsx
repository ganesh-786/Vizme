...
function ApiKeys() {
  ...
  // ---- Optional: create another key -----------------------------------------
  const handleCreateNewKey = async () => {
    setCreating(true);
    setError('');
    try {
      const response = await apiKeysAPI.create('Account API Key');
      const data = response.data;
      if (data.api_key) {
        try {
          await navigator.clipboard.writeText(data.api_key);
          showToast('API key created and copied to clipboard!', 'success', 4000);
        } catch {
          showToast('API key created. Use Copy to copy it.', 'info', 4000);
        }
      }
      setActiveKey({
        id: data.id,
      ...
      <div className="apikeys-card">
        ...
        <div className="apikeys-content">
          <div className="apikeys-grid">
            {/* Secret Key Display (always masked) — key is auto-ensured on load */}
