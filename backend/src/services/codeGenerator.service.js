...
export const generateLibraryCode = ({ apiKey, endpoint, metrics, autoTrack, customEvents }) => {
  ...
  return `(function(){
var c={k:'${apiKey}',e:'${endpoint}',m:${JSON.stringify(metrics)},a:${autoTrack},x:${customEvents}},q=[],b=[],s=10,t=5e3,bt,st=Date.now(),rt=[1e3,2e3,4e3],mxr=3;
function sm(m,r){if(!m||!m.length)return;var p={metrics:m.map(m=>({name:m.n,type:m.t,value:m.v,labels:m.l||{}}))},op={method:'POST',headers:{'Content-Type':'application/json','X-API-Key':c.k},body:JSON.stringify(p),keepalive:true};fetch(c.e,op).then(r=>{if(r.ok)return r.json();throw new Error('HTTP '+r.status)}).catch(e=>{if(r<mxr){setTimeout(()=>sm(m,r+1),rt[r])}else{if(q.length<100)q.push(...m)}}})}
...
var st=Date.now();
w.addEventListener('beforeunload',function(){var top=Math.round((Date.now()-st)/1e3);tm('time_on_page',top,{page:location.pathname});if(navigator.sendBeacon&&b.length){var p=JSON.stringify({metrics:b.map(m=>({name:m.n,type:m.t,value:m.v,labels:m.l||{}}))});navigator.sendBeacon(c.e,new Blob([p],{type:'application/json'}))}else{pb()}});
